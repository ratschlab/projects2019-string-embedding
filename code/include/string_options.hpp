...
void string_opts::add_options() {
    ...
    add_int(t_len, "-t", "--tuple-size", "number of elements in the tuple");
    add_int(len, "-l", "--str-len", "length of string to search");
    ...
    add_bool(normalize, "-N", "--normalize", "normalize cauchy output");
    add_str(dir, "-d", "--dest-dir", "normalize cauchy output");
    add_str(src, "-s", "--src-dir", "normalize cauchy output");
